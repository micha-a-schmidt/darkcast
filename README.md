...
The recasted bounds for all valid limits in `limits` are provided here. A sub-directory is provided for each model defined in `models`. These recast limits are generated by running `python examples/visible.py` and `python examples/invisible.py`. For reference details on each recast limit, please consult the `bibtex` member of the corresponding limit, e.g. `Limit(BaBar_Lees2014xha).bitex`.
