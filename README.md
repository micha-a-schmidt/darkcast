...
Darkcast is licensed under the GNU GPL version 2, or later and is copyrighted (C) 2019 by Philip Ilten, Yotam Soreq, Mike Williams, and Wei Xue.
...
The recasted bounds for all valid limits in `limits` are provided here. A sub-directory is provided for each model defined in `models`. These recast limits are generated by running `python examples/visible.py` and `python examples/invisible.py`. For reference details on each recast limit, please consult the `bibtex` member of the corresponding limit:
```python
import darkcast
limit = Limit('BaBar_Lees2014xha') 
print limit.bibtex
```
